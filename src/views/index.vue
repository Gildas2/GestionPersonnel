      ...
      try {
        ...
        if (res.data.error) {
          ...
          if (userStore.role === "admin") {
            this.$router.push({ name: "adminDashboard" }).then(() => {window.location.reload()});
          } else if (userStore.role === "Employé") {
            this.$router.push({ name: "employeDashboard" }).then(() => {window.location.reload()});
          }
        }
      } catch (err) {
        this.message = "Une erreur s'est produite lors de la connexion.";
